...
class Hyperparamters:
    # Train parameters
    num_train_epochs = 1000  # 由于训练集较少，所以模型拟合需要更多是epoch。
    print_step = 100
    batch_size = 32
    summary_step = 10
    num_saved_per_epoch = 3
    max_to_keep = 100
    logdir = 'logdir/model_07'
    file_save_model = 'model/model_07'
    file_load_model = 'model/load_01'

    # Train/Test data
    data_dir = os.path.join(pwd,'data')
    train_data = 'train_pinmu_oh_7.csv'
    test_data = 'test_pinmu_oh_7.csv'
        
    # Load vocabulcary dict
    dict_id2label,dict_label2id = load_vocabulary(os.path.join(pwd,'data','vacb7.txt'))
